...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    if (!deptRaw) {
      return res
        .status(400)
        .json({ error: "Your profile is missing department" });
    }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Call AI Service
      try {
        questions = await extractQuestionsFromTextGemini(
          text,
          difficulty,
          questionType
        );
...
router.post("/", requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const depts =
      assignedRaw.departments || assignedRaw.department || assignedRaw.dept;
    ...
    if (departments.length === 0) {
      return res
        .status(400)
        .json({ error: "assignedTo.departments is required" });
    }
    ...
    const test = new Test({
      ...
      shuffleOptions:
        typeof req.body?.shuffleOptions === "boolean"
    ...
    if (
      testObj.createdBy &&
      typeof testObj.createdBy === "object" &&
      testObj.createdBy._id
    ) {
  ...
  try {
    ...
    if (req.user.role !== "admin") {
      const hasAccess = (() => {
        ...
        const departments = Array.isArray(test.assignedTo.departments)
          ? test.assignedTo.departments
          : [];
        const inArray = departments.some(
          (d) => String(d).toLowerCase() === dept.toLowerCase()
        );
        const singleDept = test.assignedTo.department
          ? String(test.assignedTo.department)
          : null;
        const legacyDept = test.assignedTo.dept
          ? String(test.assignedTo.dept)
          : null;
        const singleMatch =
          singleDept && singleDept.toLowerCase() === dept.toLowerCase();
        const legacyMatch =
          legacyDept && legacyDept.toLowerCase() === dept.toLowerCase();
...
router.post("/:id/start", requireAuth, async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin" || userDept) {
      ...
      const singleDept =
        test.assignedTo && test.assignedTo.department
          ? String(test.assignedTo.department)
          : null;
      const legacyDept =
        test.assignedTo && test.assignedTo.dept
          ? String(test.assignedTo.dept)
          : null;
      const singleMatch =
        singleDept && singleDept.toLowerCase() === dept.toLowerCase();
      ...
      if (
        !dept ||
        !test.assignedTo ||
        !(inArray || singleMatch || legacyMatch)
      ) {
        return res.status(403).json({ error: "Not authorized for this test" });
      }
    }
    ...
    const limit = Number.isFinite(test.attemptsAllowed)
  ...
  try {
    ...
    if (attempt.test && attempt.test.questions) {
      ...
      for (const ans of answers) {
        const q = qs.find(
          (x) => x._id.toString() === (ans.questionId || "").toString()
        );
        if (q && q.correctAnswer != null) {
          if (String(ans.answer) === String(q.correctAnswer))
            score += q.points || 1;
        }
      }
    }
    ...
    attempt.percentage =
      (score / Math.max(1, attempt.test.questions.length)) * 100;
